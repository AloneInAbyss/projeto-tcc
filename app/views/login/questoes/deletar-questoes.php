    ...
    <div class="container">
        <div class="row">
            <div class="offset-lg-2 col-lg-8">
                ...
                <br><br>
                <form class="form-signin" method="post" action="/deletar-questoes">
                    <label for="inputID">Digite o ID</label>
                    <input type="text" id="inputID" class="form-control" placeholder="ID" name="idSearch" required>
                </form>
                <br>
                ...
                <table class="table table-dark">
                    ...
                    <?php 
                        if (isset($_POST['idSearch']) && $_POST['idSearch'] <> "") {
                            $result = mysqli_query($connection,"SELECT * FROM questao WHERE id =".$_POST['idSearch']);
                    ...
                    ?>
                        <tr>
							...
							<td>
								<?php echo $linha['questao']; ?>
							</td>
							<td>
								<form class="form-inline my-2 my-lg-0" method="post" action="/deletar-questoes">
									<input type="hidden" value="<?php echo $linha['id']; ?>" name="id">
									<button class="btn btn-outline-success my-2 my-sm-0" type="submit" value="ir">Deletar</button>
								</form>
							</td>
						</tr>
                    <?php 
							...
							$result = mysqli_query($connection, "SELECT * FROM questao");
							$row = mysqli_num_rows($result);
							if ($row > 0) {
                    			while ($linha = mysqli_fetch_array($result)) {
            		?>
